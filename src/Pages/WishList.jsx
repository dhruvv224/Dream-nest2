...
import { setListings, setWishList } from '../Store/Slice';
                                ...
                                <div className='slider flex transition-transform duration-500 ease-in-out' style={{ transform: `translateX(-${item.currentIndex * 300}px)` }}>
                                    {item.listingPhotoPaths.map((photoPath, photoIndex) => (
                                        <div className='w-[300px] h-[270px] flex-shrink-0' key={photoIndex}>
                                            ...
                                            <Favorite 
                                                className={`absolute top-3 right-3 text-white hover:text-gray-100 duration-100 ${item.liked ? 'text-red-500' : 'text-white'}`}
                            ...
                            <div className='flex flex-col'>
                                <Link to={`/listings/${item._id}`}>
                                    ...
                                    <div className='flex items-center'>
                                        ...
                                        <span className='inline ml-1'>{item.category === "Iconic cities" ? "Per Month" : "Per Night"}</span>
                                    </div>
                                </Link>
                            </div>
                        </div>
